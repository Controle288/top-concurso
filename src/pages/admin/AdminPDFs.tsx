...
export default function AdminPDFs() {
  ...
  const { data: pdfs = [], isLoading } = useQuery<any[]>({
    queryKey: ['pdfs'],
    queryFn: async () => {
      const { data } = await supabase.from('pdfs').select('*, concursos(titulo)').order('created_at', { ascending: false });
      return data ?? [];
    },
  });
  ...
  const invalidate = () => queryClient.invalidateQueries({ queryKey: ['pdfs'] });
    ...
    <div className="flex flex-col gap-4 py-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div><span className="text-orange-500 text-[10px] font-bold uppercase tracking-wider">Admin</span><h2 className="text-lg font-black text-white flex items-center gap-2"><FileText className="w-5 h-5 text-orange-500" /> PDFs</h2></div>
        </div>
      ...
      {showForm && (
        <div className="bg-zinc-900/70 border border-zinc-800/80 rounded-2xl p-4 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <select value={form.tipo} onChange={(e) => setForm({ ...form, tipo: e.target.value })} className="bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-3 text-sm text-zinc-200 focus:outline-none focus:border-orange-500/50"><option value="PDF">PDF</option><option value="Audio">Áudio</option><option value="Resumo">Resumo</option><option value="Lei Seca">Lei Seca</option></select>
            <select value={form.concurso_id} onChange={(e) => setForm({ ...form, concurso_id: e.target.value })} className="bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-3 text-sm text-zinc-200 focus:outline-none focus:border-orange-500/50"><option value="">Sem concurso</option>{concursos.map(c => <option key={c.id} value={c.id}>{c.titulo}</option>)}</select>
      ...
      {isLoading ? <p className="text-center text-zinc-500 py-8">Carregando...</p> : pdfs.length === 0 ? (
