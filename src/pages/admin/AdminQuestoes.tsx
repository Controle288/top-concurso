...
import type { Questao, Banca, Concurso } from '@/types';
...
export default function AdminQuestoes() {
  ...
  const { data: questoes = [], isLoading } = useQuery<Questao[]>({
    queryKey: ['questoes'],
    queryFn: async () => {
      const { data } = await supabase.from('questoes').select('*, bancas(nome)').order('created_at', { ascending: false });
      return data ?? [];
    },
  });
  ...
  const invalidate = () => queryClient.invalidateQueries({ queryKey: ['questoes'] });
  ...
  const handleEdit = (q: Questao) => {
    ...
    setForm({ enunciado: q.enunciado, alternativas: Array.isArray(alts) && alts.length === 5 ? alts : defaultAlternativas, correta: q.correta, explicacao: q.explicacao || '', banca_id: q.banca_id || '', concurso_id: q.concurso_id || '', ano: q.ano || 2026, nivel: q.nivel || 'superior' });
    setEditing(q.id);
    setShowForm(true);
  };
    ...
    <div className="flex flex-col gap-4 py-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div><span className="text-orange-500 text-[10px] font-bold uppercase tracking-wider">Admin</span><h2 className="text-lg font-black text-white">Questões</h2></div>
        </div>
      ...
      {showForm && (
        <div className="bg-zinc-900/70 border border-zinc-800/80 rounded-2xl p-4 space-y-3 max-h-[70vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1"><label className="text-[10px] text-zinc-500 font-bold uppercase">Correta</label><select value={form.correta} onChange={(e) => setForm({ ...form, correta: e.target.value })} className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-3 text-xs text-zinc-200 focus:outline-none focus:border-orange-500/50">{['A','B','C','D','E'].map(k => <option key={k} value={k}>{k}</option>)}</select></div>
            <div className="space-y-1"><label className="text-[10px] text-zinc-500 font-bold uppercase">Ano</label><input type="number" value={form.ano} onChange={(e) => setForm({ ...form, ano: Number(e.target.value) })} className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-3 text-xs text-zinc-200 focus:outline-none focus:border-orange-500/50" /></div>
            <div className="space-y-1"><label className="text-[10px] text-zinc-500 font-bold uppercase">Banca</label><select value={form.banca_id} onChange={(e) => setForm({ ...form, banca_id: e.target.value })} className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-3 text-xs text-zinc-200 focus:outline-none focus:border-orange-500/50"><option value="">Sem banca</option>{bancas.map(b => <option key={b.id} value={b.id}>{b.nome}</option>)}</select></div>
            <div className="space-y-1"><label className="text-[10px] text-zinc-500 font-bold uppercase">Concurso</label><select value={form.concurso_id} onChange={(e) => setForm({ ...form, concurso_id: e.target.value })} className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-3 text-xs text-zinc-200 focus:outline-none focus:border-orange-500/50"><option value="">Sem concurso</option>{concursos.map(c => <option key={c.id} value={c.id}>{c.titulo}</option>)}</select></div>
          </div>
      ...
      {isLoading ? <p className="text-center text-zinc-500 py-8">Carregando...</p> : questoes.length === 0 ? (
        <p className="text-center text-zinc-500 py-8">Nenhuma questão cadastrada.</p>
      ) : (
        <div className="space-y-2">{questoes.map(q => (
