...
import { useProfiles, useToggleAdmin } from '@/lib/queries/usePerfil';
...
export default function AdminUsuarios() {
  ...
  const handleToggle = (user: any) => {
    ...
    toggleAdmin.mutate({ id: user.id, role: newRole });
  };

  const filtered = usuarios.filter((u: any) => u.nome?.toLowerCase().includes(searchQuery.toLowerCase()) || u.id?.includes(searchQuery));
  ...
  return (
    <div className="flex flex-col gap-4 py-4">
      ...
      {isLoading ? <p className="text-center text-zinc-500 py-8">Carregando...</p> : filtered.length === 0 ? (
