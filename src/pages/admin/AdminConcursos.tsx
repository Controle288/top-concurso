...
export default function AdminConcursos() {
  ...
  const load = () => {
    supabase.from('concursos').select('*, bancas(nome)').order('created_at', { ascending: false }).then(({ data }) => {
  ...
  return (
    <div className="flex flex-col gap-4 py-4">
      ...
      {showForm && (
        <div className="bg-zinc-900/70 border border-zinc-800/80 rounded-2xl p-4 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <select value={form.banca_id} onChange={(e) => setForm({ ...form, banca_id: e.target.value })} className="bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-3 text-sm text-zinc-200 focus:outline-none focus:border-orange-500/50">
            ...
            <select value={form.status} onChange={(e) => setForm({ ...form, status: e.target.value as any })} className="bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-3 text-sm text-zinc-200 focus:outline-none focus:border-orange-500/50">
              <option value="aberto">Aberto</option>
              <option value="previsto">Previsto</option>
              <option value="encerrado">Encerrado</option>
            </select>
            <select value={form.nivel} onChange={(e) => setForm({ ...form, nivel: e.target.value as any })} className="bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-3 text-sm text-zinc-200 focus:outline-none focus:border-orange-500/50">
      ...
      {loading ? <p className="text-center text-zinc-500 py-8">Carregando...</p> : concursos.length === 0 ? (
            ...
            <div key={c.id} className="bg-zinc-900/60 border border-zinc-800/80 rounded-2xl p-4">
              <div className="flex justify-between items-start gap-2">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-zinc-500">{c.orgao} {(c as any).bancas?.nome && `• ${(c as any).bancas.nome}`}</p>
                </div>
